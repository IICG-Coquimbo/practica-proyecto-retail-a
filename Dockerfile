...
# 5. Librer�as de Visualizaci�n para el An�lisis de Datos
RUN pip install --no-cache-dir streamlit seaborn openpyxl

# 6. Configuración de visualización (noVNC)
COPY start-vnc.sh /usr/local/bin/start-vnc.sh
COPY supervisord.conf /etc/supervisor/conf.d/supervisord.conf

# Instalamos dos2unix, limpiamos los saltos de línea de Windows y damos permisos
